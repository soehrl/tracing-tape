...
impl<'a> Metadata<'a> {
    pub fn to_static(&self) -> Metadata<'static> {
        Metadata {
            name: (*self.name).to_owned().into(),
            target: (*self.target).to_owned().into(),
            level: self.level,
            module_path: self.module_path.as_ref().map(|p| (**p).to_owned().into()),
            file: self.file.as_ref().map(|f| (**f).to_owned().into()),
            line: self.line,
            field_names: self
                .field_names
                .iter()
...
pub enum Value<'a> {
    ...
    Uint(u64),
    Float(f64),
    Dbg(#[serde(borrow)] Cow<'a, str>),
    Str(#[serde(borrow)] Cow<'a, str>),
    Err(#[serde(borrow)] Cow<'a, str>),
}
...
impl tracing::field::Visit for Event<'_> {
    fn record_debug(&mut self, _field: &tracing::field::Field, value: &dyn std::fmt::Debug) {
        self.values.push(Value::Dbg(format!("{value:?}").into()));
    }

    fn record_str(&mut self, _field: &tracing::field::Field, value: &str) {
        self.values.push(Value::Str(Cow::Owned(value.to_owned())));
    }
    ...
    fn record_error(
        ...
        self.values
            .push(Value::Err(Cow::Owned(value.to_string().into())));
    }
...
struct SpanIntermediate {
    opened: u64,
    entrances: Vec<SpanEntrance>,
    closed: u64,
    values: Vec<Value<'static>>,
}

impl SpanIntermediate {
    fn new(opened: u64) -> Self {
        Self {
            opened,
            entrances: vec![],
            closed: 0,
...
pub struct Span<'a> {
    pub opened: u64,
    pub entrances: Vec<SpanEntrance>,
    pub closed: u64,
    pub callsite: u64,
    pub id: u64,
...
impl tracing::field::Visit for SpanIntermediate {
    fn record_debug(&mut self, _field: &tracing::field::Field, value: &dyn std::fmt::Debug) {
        self.values.push(Value::Dbg(format!("{value:?}").into()));
    }
...
#[derive(Debug, Serialize, Deserialize)]
pub enum Record<'a> {
    Callsite(#[serde(borrow)] Metadata<'a>),
    Thread(#[serde(borrow)] Thread<'a>),
    Event(#[serde(borrow)] Event<'a>),
    Span(#[serde(borrow)] Span<'a>),
}
